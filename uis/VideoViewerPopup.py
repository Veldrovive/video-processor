        ...
        if self._glo.video_config is not None:
            ...
            for val in val_list:
                if val == curr_pos:
                    ...
                    pen.setColor(QtCore.Qt.gray)
                qp.setPen(pen)
                scaled = int(round((val + self.minimum()) * (contents.width() / (self.maximum() - self.minimum()))))
                qp.drawLine(contents.x() + scaled, contents.y() + pad_amount, contents.x() + scaled, contents.y()+contents.height()-pad_amount)
...
class VideoViewerWindowV2(QtWidgets.QMainWindow):
    ...
    def setup_menu(self):
        ...
        self.add_menu("&File")
        self.add_action("&File", "Load Project Dir",
                        shortcut="Ctrl+F",
        ...
        self.add_action("&Video", "Jump Forward",
                        visible=True,
        ...
        self.add_action("&Video", "Jump Backward",
                        visible=True,
                        shortcut="Shift+left",
                        status_tip="Jump to the previous keypoint",
                        icon="./icons/rewind.png",
                        callback=jump_backwards)
        self.add_action("&Video", "Add Keypoint",
                        visible=True,
                        shortcut="Ctrl+k",
...
#         :return:
#         """
#         self.viewer = ImageViewer(glo=self.glo, vid_file=self.video_file)
#         self.viewer.frame_change_signal.connect(self.on_frame_change)
