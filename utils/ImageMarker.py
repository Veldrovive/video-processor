...
#
#     # Metric Management
#     def set_metrics(self, metrics: Optional[List[DataHolders.Metric]] = None, working_metrics: Optional[List[
#         DataHolders.Metric]] = None) -> bool:
...
class ImageMarkerV2:
    ...
    @staticmethod
    def _cast_pos(loc: Tuple[float, float], scale_factor: float) -> Tuple[int, ...]:
        """Uses the scales the point positions using the scale factor"""
        return tuple([int(round(coord * scale_factor)) for coord in loc])

    def _get_point(self, frame_num: int, landmark_def: Union[int, List[int]]):
        """Gets the position of the given landmark or centroid"""
        is_centroid = False
        if isinstance(landmark_def, list):
            point = self._glo.curr_landmarks.get_centroid(frame_num, landmark_def)
            is_centroid = True
        else:
            point = self._glo.curr_landmarks.get_landmark_locations(frame_num, landmark_def)[0]
        return point, is_centroid

    def markup_image(self, img: np.ndarray, frame_num: int) -> np.ndarray:
        """
        Gets the marked up current frame
        :param img: The current frame
        :param frame_num: The current frame number
        :return: A cv2 image of the marked up frame
        """
        img = img.copy()
        ...
        h, w, _ = img.shape
        circle_rad = int(round(max(h, w) / 450))
        visual_config = self._glo.visual_config
        landmarks = self._glo.curr_landmarks
        if visual_config.show["bound"] and landmarks.has_bbox():
            for bound_box_points in landmarks.get_bound_box_locs(frame_num):
                try:
                    lt_point, rb_point = [self._cast_pos(pos, visual_config.scale_factor) for pos in bound_box_points]
                    cv2.rectangle(img, lt_point, rb_point, (255, 0, 0), 2)
                except TypeError:
                    pass

        if visual_config.show["metrics"] and landmarks.has_landmarks():
            metrics = self._glo.metrics.metrics
            working_metric = [self._glo.metrics.working_metric]
            for metric in metrics+working_metric:
                if len(metric.landmarks) < 1:
                    # Then it's not worth doing anything
                    continue
                # We store the last point in order to draw a line to the current
                last_point = self._cast_pos(self._get_point(frame_num, metric.landmarks[0])[0], visual_config.scale_factor)
                for landmark_def in metric.landmarks:
                    curr_point = self._cast_pos(self._get_point(frame_num, landmark_def)[0], visual_config.scale_factor)
                    cv2.line(img, last_point, curr_point, visual_config.highlight_color, 1)
                    # Put an outline around the landmark
                    cv2.circle(img=img, center=last_point, radius=circle_rad+1, color=visual_config.highlight_color, thickness=1)
                    last_point = curr_point
                cv2.circle(img=img, center=last_point, radius=circle_rad + 1, color=visual_config.highlight_color, thickness=1)
                if metric.type == DataHolders.MetricType.AREA:
                    # If the metric is an area, then close the polygon
                    curr_point = self._cast_pos(self._get_point(frame_num, metric.landmarks[0])[0], visual_config.scale_factor)
                    cv2.line(img, last_point, curr_point, visual_config.highlight_color, 1)

        if visual_config.show["land"] and landmarks.has_landmarks():
            for landmark in landmarks.get_landmarks(frame_num):
                loc, group, index = landmark
                if index in visual_config.excluded_landmarks:
                    continue
                color = (0, 0, 0)
                group_colors = visual_config.group_colors
                if group in group_colors:
                    color = group_colors[group]
                if index in self._glo.metrics.working_metric.landmarks:
                    color = visual_config.highlight_color
                true_pos = self._cast_pos(loc, visual_config.scale_factor)
                cv2.circle(img=img, center=true_pos, radius=circle_rad, color=color, thickness=-1)
                cv2.putText(img,
                            str(index),
                            (true_pos[0] - 2, true_pos[1] - 2),
