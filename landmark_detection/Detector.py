...
from landmark_detection.face_alignment.models import FAN, LightningFAN
...
class LightningFANDetector(QtCore.QThread):
    _glo: Globals

    _face_alignment_net: LightningFAN = None
    ...
    def FAN_model_path(self):
        return self._glo.model_config.model_path

    @property
    def detector_model_path(self):
        return self._glo.model_config.s3fd_path

    def __init__(self, frames_map: Dict[str, Set], parallel_frames: int=1, det_thresh: float=0.3):
        ...
        self._num_parallel_frames = parallel_frames
    ...
    def load_model(self):
        try:
            print("Loading Lightning FAN")
            self._face_alignment_net = LightningFAN.load_from_checkpoint(self.FAN_model_path)
        except Exception as e:
            print("Loading original FAN")
            self._face_alignment_net = LightningFAN(FAN_path=self.FAN_model_path)
        except Exception as e:
            print("Error was: ", e)
        self._face_alignment_net.eval()
